    ...
    return (
        <div className="flex flex-col p-6 items-center">
            ...
            <div className="flex pt-2 justify-center gap-2">
                ...
                <div className="mx-2 text-center">&middot;</div>
                {!!user.company && <div>🏢 {user.company.substring(1)}</div>}
            </div>
            {!!user.blog && <div className="flex pt-2 text-xl"><a className="text-cyan-600" href={user.blog} target="_blank" rel="noopener noreferrer">{user.blog.substring(8)}</a></div>}
        </div>
    );
};
